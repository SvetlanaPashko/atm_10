...
const contributorCard = aboutPage.cards.contributor('Doug Parker');
...
describe('smoke suit', () => {
    ...
    it('open page from search results', async () => {
        ...
        await homePage.header.searchInput.setValue('create');
    ...
    it('check that hover contains elements on About page', async () => {
        ...
        expect(await contributorCard.icon('viewBio') .isDisplayed()).to.be.false;
        expect(await contributorCard.icon('gitHub') .isDisplayed()).to.be.false;
        expect(await contributorCard.icon('twitter') .isDisplayed()).to.be.false;

        await contributorCard.hover();
        expect(await contributorCard.icon('viewBio').isDisplayed()).to.be.true;
        expect(await contributorCard.icon('gitHub') .isDisplayed()).to.be.true;
        expect(await contributorCard.icon('twitter') .isDisplayed()).to.be.true;
    })
